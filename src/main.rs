...
async fn check_issues() -> Result<impl IntoResponse, String> {
    ...
    let telegram_bot_token = env::var("TELEGRAM_BOT_TOKEN").map_err(|_| "Missing Telegram bot token")?;
    ...
    let issues: Vec<Issue> = client
        .get("https://api.github.com/repos/paradigmxyz/reth/issues")
    ...
    let recent_issues: Vec<_> = issues.into_iter()
        .filter(|issue| {
